...
cfd_solver::cfd_solver( float xsz, float ysz, float zsz )
{
	...
	uint vbo_size			=	sizeof(float3) * prt_num;

	//	create vbo for particles :
	m_position.resize( prt_num );
	...
	//	alloc CUDA buffers :
	CUDA_SAFE_CALL( cudaMalloc( &d_position[0], total_grid_size * sizeof(float3) ) );
	...
	CUDA_SAFE_CALL( cudaMalloc( &d_position[1], total_grid_size * sizeof(float3) ) );
	...
	CUDA_SAFE_CALL( cudaMemset( d_position[0], 0, total_grid_size * sizeof(float3) ) );
	...
	CUDA_SAFE_CALL( cudaMemset( d_position[1], 0, total_grid_size * sizeof(float3) ) );
