...
namespace Firmware {
	public class ITG3200 {
		...
		#region CONSTANTS
		const byte  ITG3200_ADDR_AD0_HIGH = 0x69;   //AD0=1 0x69 I2C address when AD0 is connected to HIGH ( VCC ) - default for sparkfun breakout
		const byte  ITG3200_ADDR_AD0_LOW  = 0x68;   //AD0=0 0x68 I2C address when AD0 is connected to LOW ( GND )
		...
		const byte  GYROSTART_UP_DELAY = 70;    // 50ms from gyro startup + 20ms register r/w startup

		/* ---- Registers ---- */
		const byte  WHO_AM_I          = 0x00;  // RW   SETUP: I2C address   
		const byte  SMPLRT_DIV        = 0x15;  // RW   SETUP: Sample Rate Divider       
		const byte  DLPF_FS           = 0x16;  // RW   SETUP: Digital Low Pass Filter/ Full Scale range
		const byte  INT_CFG           = 0x17;  // RW   Interrupt: Configuration
		const byte  INT_STATUS        = 0x1A;  // R	Interrupt: Status
		const byte  TEMP_OUT          = 0x1B;  // R	SENSOR: Temperature 2bytes
		const byte  GYRO_XOUT         = 0x1D;  // R	SENSOR: Gyro X 2bytes  
		const byte  GYRO_YOUT         = 0x1F;  // R	SENSOR: Gyro Y 2bytes
		const byte  GYRO_ZOUT         = 0x21;  // R	SENSOR: Gyro Z 2bytes
		const byte  PWR_MGM           = 0x3E;  // RW	Power Management

		/* ---- bit maps ---- */
		const byte  DLPFFS_FS_SEL            = 0x18;  // 00011000
		const byte  DLPFFS_DLPF_CFG          = 0x07;  // 00000111
		const byte  INTCFG_ACTL              = 0x80;  // 10000000
		const byte  INTCFG_OPEN              = 0x40;  // 01000000
		const byte  INTCFG_LATCH_INT_EN      = 0x20;  // 00100000
		const byte  INTCFG_INT_ANYRD_2CLEAR  = 0x10;  // 00010000
		const byte  INTCFG_ITG_RDY_EN        = 0x04;  // 00000100
		const byte  INTCFG_RAW_RDY_EN        = 0x01;  // 00000001
		const byte  INTSTATUS_ITG_RDY        = 0x04;  // 00000100
		const byte  INTSTATUS_RAW_DATA_RDY   = 0x01;  // 00000001
		const byte  PWRMGM_HRESET            = 0x80;  // 10000000
		const byte  PWRMGM_SLEEP             = 0x40;  // 01000000
		const byte  PWRMGM_STBY_XG           = 0x20;  // 00100000
		const byte  PWRMGM_STBY_YG           = 0x10;  // 00010000
		const byte  PWRMGM_STBY_ZG           = 0x08;  // 00001000
		...
		/************************************/
		// Sample Rate Divider
		const byte  NOSRDIVIDER        = 0; // default    FsampleHz=SampleRateHz/( divider+1 )
		// Gyro Full Scale Range
		const byte  RANGE2000          = 3;   // default
		// Digital Low Pass Filter BandWidth and SampleRate
		const byte  BW256_SR8          = 0;   // default    256Khz BW and 8Khz SR
		const byte  BW188_SR1          = 1;
		const byte  BW098_SR1          = 2;
		const byte  BW042_SR1          = 3;
		const byte  BW020_SR1          = 4;
		const byte  BW010_SR1          = 5;
		const byte  BW005_SR1          = 6;
		// Interrupt Active logic lvl
		const byte  ACTIVE_ONHIGH      = 0; // default
		const byte  ACTIVE_ONLOW       = 1;
		// Interrupt drive type
		const byte  PUSH_PULL          = 0; // default
		const byte  OPEN_DRAIN         = 1;
		// Interrupt Latch mode
		const byte  PULSE_50US         = 0; // default
		const byte  UNTIL_INT_CLEARED  = 1;
		// Interrupt Latch clear method
		const byte  READ_STATUSREG     = 0; // default
		const byte  READ_ANYREG        = 1;
		// Power management
		const byte  NORMAL             = 0; // default
		const byte  STANDBY            = 1;
		// Clock Source - user parameters
		const byte  INTERNALOSC        = 0;   // default
		const byte  PLL_XGYRO_REF      = 1;
		const byte  PLL_YGYRO_REF      = 2;
		const byte  PLL_ZGYRO_REF      = 3;
		const byte  PLL_EXTERNAL32     = 4;   // 32.768 kHz
		const byte  PLL_EXTERNAL19     = 5;   // 19.2 Mhz
	#endregion
